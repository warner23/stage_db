...
function sendFileToShowServer(formData,status, ele_id)
{
    ...
    processData: false,
        ...
        success: function(data){
            ...
            //alert(ele_id);
            preview = ('<img src="'+info.name+'" class="img-responsive showsImg" value="'+info.id+'" id="preview-'+ele_id+'">');
...
function sendFileToCSVServer(formData,status, ele_id, table)
{
    //console.log(formData);
      var uploadURL ="WICore/WIClass/CSVUpload.php"; //Upload URL
    var extraData ={}; //Extra Data.
    var jqXHR=$.ajax({
            xhr: function() {
            var xhrobj = $.ajaxSettings.xhr();
    ...
    processData: false,
        ...
        success: function(data){
            ...
            alert(info.name);
            status.setProgress(100);
            $("#status").append("File upload Done<br>").fadeOut(3000);
            $(".dragandrophandler").remove()
            //alert(ele_id);
            preview = ('<a href="javascript:void(0);" onclick="WITheatres.InstallCSV(`'+info.name+'`, `'+table+'`);" value="'+info.name+'" id="preview-'+ele_id+'" class="btn">Import</a>');
            $("#"+ele_id).append(preview);
            //$("#uploadOptions.well").removeClass("on").addClass("off");  
        }
    }); 
...
function sendFileToShowTrailerServer(formData,status, ele_id)
{
      var uploadURL ="WICore/WIClass/ShowTrailerImageUpload.php"; //Upload URL
    var extraData ={}; //Extra Data.
    var jqXHR=$.ajax({
            xhr: function() {
            var xhrobj = $.ajaxSettings.xhr();
...
function showCreationhandleFileUpload(files,obj, dir, ele_id)
{
   for (var i = 0; i < files.length; i++) 
   {
        ...
        }else if(dir === "show_trailer") {
             sendFileToShowTrailerServer(fd,status, ele_id);
        }else if(dir === "csv") {
             sendFileToCSVServer(fd,status, ele_id);
        }else{
            sendFileToServer(fd,status);
        }
...
function BulkStagehandleFileUpload(files,obj, dir, ele_id, folder)
{
   for (var i = 0; i < files.length; i++) 
   {
        var fd = new FormData();
        ...
        var status = new createStatusbar(obj); //Using this we can set progress.
        status.setFileNameSize(files[i].name,files[i].size);
        if (dir === "bulkPersonUpload") {
             sendFileToPersonServer(fd,status, ele_id);
        }else if (dir === "bulkCompanyUpload") {
             sendFileToCompanyServer(fd,status, ele_id);
        }else if (dir === "bulkShowUpload") {
             sendFileToShowServer(fd,status, ele_id);
        }else if (dir === "bulkTheatreUpload") {
             sendFileToTheatreServer(fd,status, ele_id);
        }
...
function showBulkhandleFileUpload(files,obj, dir, ele_id, table)
{
   for (var i = 0; i < files.length; i++) 
   {
        var fd = new FormData();
        ...
        status.setFileNameSize(files[i].name,files[i].size);
        if (dir === "csv") {
             sendFileToCSVServer(fd,status, ele_id, table);
        }else{
