...
/*
        ...
        $mod_drop->mod_name($empty, $mod_name);
          ...
          public function dropModule($mod_name)
     {
...
/*
        ...
        echo $mod_drop;*/
        $mod_drop = new $mod_drop;
        $mod_drop->editContents($mod_name);
...
/*        $sql = "SELECT * FROM `wi_mod` WHERE mod_status = :mod_status ORDER BY `mod_id` ASC LIMIT :page, :item_per_page";
        ...
        //var_dump($resy);
        echo '<ul id="draggableMod" class="ui-widget-header">';
        foreach ($resy as $res) {
            echo '<li id="'.$res['module_name'] . '" title="'.$res['module_name'] . '" class="ui-draggable ui-draggable-handle '.$res['mod_font'] . '" draggable="true">'.$res['module_name'] . '</li>';
        }
        echo '</ul>';
         $Pagination = $this->Page->Pagination($item_per_page, $page_number, $rows, $total_pages, $onclick, $JsClass);
    //print_r($Pagination);
   ...
   // echo $mlang;
    ...
    public function createMod($contents, $mod_name, $mod_drop, $element)
    {
        ...
        /*foreach ($element as $ele ) {
            include_once dirname(dirname(dirname(__FILE__))) . '/WIModule/Elements/' . $ele . '/'. $ele . '.php';
            $modEle = new $ele;
            $newElement = $modEle->Feature();
        }*/

        /*
        Use the structure of the element in order to construct the db contents
        so I need all the fields of the element to add columns to the db to make saving them simplier
        */

        $newElement = implode(",",$element); 
            ...
            "edit_mod" => $contents,
            "edit_page" => $contents,
            "mod_name_function" => $newElement
        )); 
             $moduleId = $this->WIdb->lastInsertId();
...
class '. $result[0]['name'] . '
{
        ...
        public function editMod()
    {
        ...
        $this->Bootstrap->Edit($page, $element, $Id);
    ...
    public function mod_name($module, $page)
    {
        $Id = ' .$moduleId. ';
        $element = $this->WIdb->select(
                    "SELECT `mod_name_function` FROM `wi_modules`
                     WHERE `id` = :id",
                     array(
                       "id" => $Id
                     )
                  );
        $this->Bootstrap->createModule($module, $page, $element, $Id);
    }
        ...
        public function moduleImg($page_id, $column)
    {
        ...
        $res = $query1->fetch(PDO::FETCH_ASSOC);
            ...
            echo '<div class="col-lg-8 col-md-3 col-sm-2">
