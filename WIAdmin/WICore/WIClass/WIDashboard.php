...
class WIDashboard
{
        ...
        public function WINotifications()
    {
       ...
       $page_position = (($page_number-1) * $item_per_page);
        ...
        while($res = $query->fetchAll() )
        {
            //var_dump($res);
            foreach ($res as $key => $value) {
               $username = self::GetUsername($value['user']);
               // $completed =  $value['completed'];
                //print_r($value);
                     echo '<li>
                     <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                     <i class="fa fa-bell-o text-aqua"></i> ' . $value['opperation'] . '
                                </div>
                                <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                                <i class="fa fa-users text-aqua"></i> ' . $username . '
                                </div>
                                <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                                <i class="text-aqua"></i> ' . $value['date'] . '
                                </div>
                  </li>';
    ...
    {
        $result = $this->WIdb->select("SELECT `username` FROM `wi_members` WHERE `user_id`=:user", array("user" => $user_id)
    );
