...
class WICast {
    ...
    public function __construct() {
     ...
     public function addingCastPerson($show_id, $show_name, $character_name, $actor_id, $img, $role)
  {

        //echo "shid". $show_id;
    $this->WIdb->insert('wi_theatre_cast', array(
            "actor_id"     => $actor_id,
        ...
        $sql0 = "UPDATE `wi_shows` SET  `cast_id` =:show_id WHERE `id` =:show_id";
   ...
   public function editCastingPerson($person_id, $show_id, $show_name, $character_name, $cast_id, $actor_id, $img, $role)
  {
    ...
    $sql = "SELECT * FROM `wi_theatre_cast` WHERE `id`=:cast_id";
    ...
    if (empty($res['id'])) {
      ...
      $sql0 = "UPDATE `wi_theatre_cast` SET `actor_id`=:actor_id, `character_name`=:character_name, `show_id`=:show_id, `photo`=:img, `play_name`=:show_name WHERE `id`=:cast_id";
  ...
  Public function ViewEditShowingCast()
  {
    if(isset($_POST["page"])){
        ...
        <div class="col-xs-6">
          ...
          <div class="middle-bit-show">
                   ...
                   <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                       <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 cast" id="casty">';self::editGetCast($show['id']); echo '</div> 
                       </div>
                  <div class="end-bit">
                   </div>
                    </div>
          ...
          <div class="middle-bit-show">
                    <div class="row">
                      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                         <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                             <legend>Cast</legend>
                             <div id="cast">';self::getCast($show['id']); echo '</div>
                      </div>
                        </div>
     ...
     $sql = "SELECT * FROM `wi_theatre_cast` WHERE `show_id`=:cast_id";
        ...
        foreach ($res as $cast ) {
            ...
            if (empty($photo)) {
               ...
               echo '<li class="col-md-12 casting" id="cast_' .$cast['actor_id'] . '">
                <div id="edit_casting_' .$cast['actor_id'] . '">
                ...
                <div class="pb">Played By :</div><div class="playedBy" id="playedBy">' . self::PlayedBy($cast['actor_id'], $id) . '<div id="select_actor"></div></div></div></li>';
            }
           echo '</ul>'; 
  ...
  public function playedBy($actor_id)
   {
    $sql = "SELECT * FROM `wi_theatre_person` WHERE `id`=:actor_id";
     ...
     public function deleteCastPerson($id)
  {
    $sql = "DELETE FROM `wi_theatre_cast` WHERE `id`=:id";
    $query = $this->WIdb->prepare($sql);
    $query->bindParam(':id', $id, PDO::PARAM_INT);
    $query->execute();

    $msg = "You have successfully removed this person from cast";
     ...
     public function editGetCast($id)
  {
    //echo "id".$id;
     $sql = "SELECT * FROM `wi_theatre_cast` WHERE `show_id`=:cast_id";
        $query = $this->WIdb->prepare($sql);
        ...
        foreach ($res as $cast ) {
            ...
            if (empty($photo)) {
                ...
                <div class="pbc">Played By :</div><div class="playedBy" id="playedBy">' . self::editplayedBy($cast['actor_id'], $cast['show_id']) . '</div> <a href="javascript:void(0);" onclick="WICast.saveEditCharacter(' .  $cast['id'] . ');" class="btn">Save</a><a href="javascript:void(0);" onclick="WICast.DeleteCharacter(' .  $cast['id'] . ');" class="btn">Delete</a></li>';
            }
           echo '</ul>'; 
