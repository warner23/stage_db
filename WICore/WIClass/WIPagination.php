...
class WIPagination {        
        ...
        public function __construct()
        {
         ...
         public function Pagination($item_per_page, $current_page, $total_records, $total_pages, $onclick, $JsClass)
    {
         $pagination = '';
    if($total_pages > 0 && $total_pages != 1 && $current_page <= $total_pages){ //verify total pages and current page number
        ...
        if($current_page > 1){
            ...
            $pagination .= '<li class="first"><a href="javascript:void(0);" onclick="' .$JsClass .'.'  . $onclick . '(1);" class="pagination" data-page="1" title="First">&laquo;</a></li>'; //first link
            $pagination .= '<li><a href="javascript:void(0);" class="pagination" onclick="' .$JsClass .'.'  . $onclick . '();"  data-page="'.$previous_link.'" title="Previous">&lt;</a></li>'; //previous link
                for($i = ($current_page-2); $i < $current_page; $i++){ //Create left-hand side links
                    if($i > 0){
                        $pagination .= '<li><a href="javascript:void(0);" class="pagination" onclick="' .$JsClass .'.'  . $onclick . '('.$i.');" data-page="'.$i.'" title="Page'.$i.'">'.$i.'</a></li>';
                    }
                }   
        ...
        for($i = $current_page+1; $i < $right_links ; $i++){ //create right-hand side links
            if($i<=$total_pages){
                $pagination .= '<li><a href="javascript:void(0);"  onclick="' .$JsClass .'.'  . $onclick . '('.$i.');" class="pagination" data-page="'.$i.'" title="Page '.$i.'">'.$i.'</a></li>';
            }
        }
        if($current_page < $total_pages){ 
                $next_link = ($i > $total_pages) ? $total_pages : $i;
                $pagination .= '<li><a href="javascript:void(0);" class="pagination" onclick="' .$JsClass .'.'  . $onclick . '('.$next_link.');"  data-page="'.$next_link.'" title="Next">&gt;</a></li>'; //next link
                $pagination .= '<li class="last"><a href="javascript:void(0);" onclick="' .$JsClass .'.'  . $onclick . '('.$total_pages.');"  class="pagination" data-page="'.$total_pages.'" title="Last">&raquo;</a></li>'; //last link
        }
