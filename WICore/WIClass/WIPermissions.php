...
class WIPermissions
{
    ...
    public function hasPermission($user_id, $group, $perm_name, $column)
    {
      if ($user_id === NULL) {
        return false;
      }
      $role = $this->WIdb->select('SELECT `user_role` FROM `wi_members` WHERE `user_id` = :user_id', array('user_id' => $user_id) );
      //var_dump($role);
      $role_id = $role[0]['user_role'];
      //echo $role_id;
      //echo $group;
      //echo $perm_name;
      $permissions = $this->WIdb->select("SELECT * FROM `wi_user_permissions` WHERE `role_id`=:role_id AND `group`=:group AND `perm_name`=:perm_name", array("role_id" => $role_id, "group" => $group, "perm_name" => $perm_name) );
      //var_dump($permissions);
        //echo $permissions[0][$column];
      $haspermission = $permissions[0][$column];
        if ($haspermission > 0) {
          return true;
        }else{
          return false;
...
FROM wi_user_group";
        ...
        echo '<select id="groups' . $role_id . '" class="col-md-3">';
