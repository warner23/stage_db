...
class WIProfile
{
	...
	public function userDetails($userId, $column)
	{
      	 ...
      	 public function User_pic($userId)
	 {
	    ...
	    while ($result = $query->fetchAll(PDO::FETCH_ASSOC) ) {
	    	...
	    	if(!empty($result[0]["avatar"])){
			 echo '<a href="#" onclick="profile.photo()" class="btn"><img class="profile" src="../WIAdmin/WIMedia/Img/avator/' . $result[0]["avatar"] . '" width="218px" />' . WILang::get("change_pic") . '</a>';
			
		} else {
		  echo '<a href="#"" onclick="profile.photo()" class="btn"><img class="profile" src="../WIAdmin/WIMedia/Img/avator/image01.jpg" width="218px" /><div class="pic">' . WILang::get("change_pic") . '</div></a>';
		}
	 ...
	 public function Display_name($userId)
	 {
	   ...
	   if (count($res) > 0 || !empty($res['first_name'])) {
